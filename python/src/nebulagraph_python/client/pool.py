...
class NebulaPool:
    ...
    Safe for thread-level concurrency, not asyncio-level.
    ...
    def __init__(
        ...
        *,
        ...
        self.fulfill_pool(new_pool=True)

    def fulfill_pool(self, locked: bool = False, new_pool: bool = False):
        ...
        def _inner() -> None:
            ...
            for _ in range(
                len(self.hosts)
                if (new_pool and self.pool_config.strictly_server_healthy)
                else to_fill_num
            ):
            ...
            # Truncate the client list to the max size
            # when new pool is created and strictly_server_healthy is True
            self._clients = self._clients[: self.pool_config.max_client_size]
    ...
    def get_client(self) -> NebulaClient:
        ...
        Raises:
        ------
            InternalError: If all clients are in use
        """
        ...
        with self._lock:
            ...
            while time.time() - start_time < self.pool_config.max_wait:
                ...
                if client:
                    return client
            raise InternalError("All clients are in use")
