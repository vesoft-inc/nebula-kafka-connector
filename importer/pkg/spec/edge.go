...
const (
	fmtEdgePrefixStatement = "USE %s INSERT EDGE %s %s" // "USE graph INSERT EDGE name (props),(props)"
	fmtEdgeValueStatement  = "(%s)-[%s]->(%s)"          // "(srcID)-[props]->(dstID)"
)
...
func (e *Edge) ValueStatement(graphName string, records ...Record) (string, error) {
	values := make([]string, 0, len(records))
	for _, record := range records {
		srcStatement, err := e.Src.ValueStatement(record)
		if err != nil {
			return "", e.importError(err).SetGraphName(graphName)
		}
		dstStatement, err := e.Dst.ValueStatement(record)
		if err != nil {
			return "", e.importError(err).SetGraphName(graphName)
		}
		propsStatement, err := e.Props.ValueStatement(record)
		if err != nil {
			return "", e.importError(err).SetGraphName(graphName)
		}
		values = append(values, fmt.Sprintf(fmtEdgeValueStatement, srcStatement, propsStatement, dstStatement))
	}
	return fmt.Sprintf(fmtEdgePrefixStatement, graphName, e.Name, strings.Join(values, ", ")), nil
}
...
func (n *EdgeNodeRef) ValueStatement(record Record) (string, error) {
	return n.ID.ValueStatement(record)
}
