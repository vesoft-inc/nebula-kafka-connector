...
var _ = Describe("Edge", func() {
	...
	Describe(".ValueStatement", func() {
		When("no props", func() {
			...
			It("one record", func() {
				statement, err := edge.ValueStatement("graphName", []string{"1", "id1", "1.1", "str1"})
				Expect(err).NotTo(HaveOccurred())
				Expect(statement).To(Equal("USE graphName INSERT EDGE name ({id: 1})-[{}]->({id: \"id1\"})"))
			})

			It("two record", func() {
				statement, err := edge.ValueStatement("graphName", []string{"1", "id1", "1.1", "str1"}, []string{"2", "id2", "2.2", "str2"})
				Expect(err).NotTo(HaveOccurred())
				Expect(statement).To(Equal("USE graphName INSERT EDGE name ({id: 1})-[{}]->({id: \"id1\"}), ({id: 2})-[{}]->({id: \"id2\"})"))
			})

			It("src failed", func() {
				statement, err := edge.ValueStatement("graphName", []string{})
				Expect(err).To(HaveOccurred())
			...
			It("dst failed", func() {
				statement, err := edge.ValueStatement("graphName", []string{"1"})
				Expect(err).To(HaveOccurred())
				Expect(stderrors.Is(err, errors.ErrNoRecord)).To(BeTrue())
				Expect(statement).To(BeEmpty())
			})
		...
		When("one prop", func() {
			...
			It("props failed", func() {
				statement, err := edge.ValueStatement("graphName", []string{"1", "id1"})
				Expect(err).To(HaveOccurred())
				Expect(stderrors.Is(err, errors.ErrNoRecord)).To(BeTrue())
				Expect(statement).To(BeEmpty())
			})
