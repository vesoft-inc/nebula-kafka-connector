...
var _ = Describe("Graph", func() {
	...
	Describe(".Validate", func() {
		It("no name", func() {
			graph := NewGraph("")
			err := graph.Validate()
			Expect(stderrors.Is(err, errors.ErrNoGraphName)).To(BeTrue())
		})
	...
	Describe(".NodeStatement", func() {
		...
		It("success", func() {
			node := graph.Nodes[0]
			statement, err := graph.NodeStatement(node, []string{"1"})
			Expect(err).NotTo(HaveOccurred())
			Expect(statement).To(Equal("USE graphName INSERT NODE node1 ({id: 1})"))
		})

		It("failed", func() {
			node := graph.Nodes[0]
			statement, err := graph.NodeStatement(node, []string{})
	...
	Describe(".EdgeStatement", func() {
		...
		It("success", func() {
			edge := graph.Edges[0]
			statement, err := graph.EdgeStatement(edge, []string{"1", "2"})
			Expect(err).NotTo(HaveOccurred())
			Expect(statement).To(Equal("USE graphName INSERT EDGE edge1 ({id: 1})-[{}]->({id: 2})"))
		})

		It("failed", func() {
			edge := graph.Edges[0]
			statement, err := graph.EdgeStatement(edge, []string{})
			Expect(err).To(HaveOccurred())
			Expect(stderrors.Is(err, errors.ErrNoRecord)).To(BeTrue())
			Expect(statement).To(Equal(""))
