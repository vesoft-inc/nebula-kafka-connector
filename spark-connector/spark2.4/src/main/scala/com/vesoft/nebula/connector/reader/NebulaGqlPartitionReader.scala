...
class NebulaGqlPartitionReader extends InputPartitionReader[InternalRow] {
  ...
  override def next(): Boolean = this.resultSet.hasNext
  ...
  override def close(): Unit = graphProvider.close()
