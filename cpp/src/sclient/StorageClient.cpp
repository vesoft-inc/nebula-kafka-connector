...
StorageClient::StorageClient(const std::vector<std::string>& metaAddrs,
    ...
    clientsMan_ = std::make_shared<
            thrift::ThriftClientManager<storage::cpp2::GraphStorageServiceAsyncClient>>(
...
ScanEdgeIter StorageClient::scanEdgeWithPart(std::string spaceName,
    ...
    req->set_parts(
            std::unordered_map<PartitionID, storage::cpp2::ScanCursor>{{partId, scanCursor}});
    ...
    folly::via(
            ...
            [evb,
             ...
             this]() mutable {
                ...
                auto client =
                        clientsMan_->client(host, evb, false, sConfig_.clientTimeoutInMs_);
                ...
                remoteFunc(client.get(), request.second)
                        ...
                        .then([pro = std::move(pro), host](folly::Try<Response>&& t) mutable {
                            ...
                            if (t.hasException()) {
                                ...
                                LOG(ERROR) << "RpcResponse exception: "
