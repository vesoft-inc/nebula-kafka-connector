...
protected:
    static void prepare() {
        ...
        pool.init({kServerHost ":9669"}, nebula::client::Config{0, 0, 10, 0, "", true});
        ...
        ASSERT_EQ(result.errorCode, nebula::client::ErrorCode::SUCCEEDED) << *result.errorMsg;

        auto result2 = session.execute("CREATE EDGE IF NOT EXISTS like(likeness int)");
        ASSERT_EQ(result2.errorCode, nebula::client::ErrorCode::SUCCEEDED) << *result2.errorMsg;
    ...
    static void runOnce(nebula::client::MetaClient &c) {
        ...
        EXPECT_EQ(parts, (std::vector<nebula::client::PartitionID>{1}));

        auto ret4 = c.getPartLeaderFromCache(spaceId, 1);
        ASSERT_TRUE(ret4.first);
        EXPECT_EQ(ret4.second, nebula::client::HostAddr(kServerHost, 9779));
    }

    static void runGetParts(nebula::client::StorageClient &c) {
        auto parts = c.getParts("storage_client_test");
        EXPECT_EQ(parts, (std::vector<nebula::client::PartitionID>{1}));
    }

    static void runScanEdgeWithPart(nebula::client::StorageClient &c) {
        LOG(INFO) << "run with goodScanIter";
        {
            auto goodScanIter = c.scanEdgeWithPart("storage_client_test",
                                                   ...
                                                   true,
                                                   true);
            nebula::client::DataSet expected({"like.likeness"});
            expected.emplace_back(nebula::client::List({78}));
            expected.emplace_back(nebula::client::List({99}));
            expected.emplace_back(nebula::client::List({43}));
            expected.emplace_back(nebula::client::List({56}));
            expected.emplace_back(nebula::client::List({-13}));
            expected.emplace_back(nebula::client::List({431}));
            expected.emplace_back(nebula::client::List({457}));

            nebula::client::DataSet got;
            int scanNum = 0;
            while (goodScanIter.hasNext()) {
                nebula::client::DataSet ds = goodScanIter.next();
        ...
        LOG(INFO) << "run with badScanIter";
        {
                                                  ...
                                                  true);
            {
                ...
                nebula::client::DataSet ds = badScanIter.next();
                EXPECT_EQ(ds, nebula::client::DataSet());
...
TEST_F(StorageClientTest, SSL) {
    ...
    prepare();
    nebula::client::MConfig mConfig{1000, 60 * 1000, true, ""};
    nebula::client::SConfig sConfig{1000, 60 * 1000, true, ""};
    nebula::client::StorageClient c({kServerHost ":9559"}, mConfig, sConfig);
