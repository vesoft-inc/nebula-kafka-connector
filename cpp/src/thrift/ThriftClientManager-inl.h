...
std::shared_ptr<ClientType> ThriftClientManager<ClientType>::client(const HostAddr& host,
    ...
    if (it != clientMap_->end()) {
        do {
            auto channel = dynamic_cast<apache::thrift::HeaderClientChannel*>(
            ...
            if (transport == nullptr || transport->hangup()) {
                clientMap_->erase(it);
                VLOG(2) << "Transport is closed by peers " << transport
                        << " for host: " << host;
                break;
            }
    ...
    evb->runImmediatelyOrRunInEventBaseThreadAndWait([this, &socket, evb, resolved]() {
        if (enableSSL_) {
            socket = folly::AsyncSSLSocket::newSocket(nebula::client::createSSLContext(CAPath_),
                                                      evb);
            socket->connect(nullptr, resolved.host, resolved.port, connTimeoutInMs_);
        } else {
            socket = folly::AsyncSocket::newSocket(
                    evb, resolved.host, resolved.port, connTimeoutInMs_);
        }
    });
    ...
    std::shared_ptr<ClientType> client(
            new ClientType(std::move(headerClientChannel)), [evb](auto* p) {
