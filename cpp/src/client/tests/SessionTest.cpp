...
protected:
    static void runOnce(nebula::client::ConnectionPool& pool) {
        ...
        ASSERT_EQ(result.errorCode, nebula::client::ErrorCode::SUCCEEDED);
        ...
        session.asyncExecute("YIELD 1", [&b](auto&& cbResult) {
            ASSERT_EQ(cbResult.errorCode, nebula::client::ErrorCode::SUCCEEDED);
            nebula::client::DataSet cbExpected({"1"});
            cbExpected.emplace_back(nebula::client::List({1}));
        ...
        // retry connection
        ASSERT_EQ(session.retryConnect(), nebula::client::ErrorCode::SUCCEEDED);
        ...
        // check release
        result = session.execute("YIELD 1");
        ASSERT_EQ(result.errorCode, nebula::client::ErrorCode::E_DISCONNECTED);

        // async execute
        folly::Baton<> b2;
        session.asyncExecute("YIELD 1", [&b2](auto&& cbResult) {
            ASSERT_EQ(cbResult.errorCode, nebula::client::ErrorCode::E_DISCONNECTED);
            b2.post();
        });
        b2.wait();
...
TEST_F(SessionTest, OverUse) {
    ...
    std::vector<nebula::client::Session> sessions;
...
TEST_F(SessionTest, Data) {
    nebula::client::ConnectionPool pool;
    nebula::client::Config c{10, 0, 300, 0, "", false};
    pool.init({kServerHost ":9669"}, c);
    auto session = pool.getSession("root", "nebula");
    ASSERT_TRUE(session.valid());
    ...
    ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::SUCCEEDED) << *resp.errorMsg;

    ::sleep(30);

    resp = session.execute(
            "INSERT VERTEX geo VALUES 'v101':(ST_Point(3, 8), ST_Point(4, 6), "
            "ST_GeogFromText('LINESTRING(0 1, 2 3)'), ST_GeogFromText('POLYGON((0 1, 1 2, 2 3, "
            "0 1))'))");
    ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::SUCCEEDED) << *resp.errorMsg;

    // execute
    resp = session.execute(
            "FETCH PROP ON geo 'v101' YIELD geo.any_shape, geo.only_point, "
            "geo.only_lineString, "
            "geo.only_polygon");
    ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::SUCCEEDED);
    nebula::client::DataSet expected(
            {"geo.any_shape", "geo.only_point", "geo.only_lineString", "geo.only_polygon"});
    nebula::client::Geography geogPoint1(
            nebula::client::Point(nebula::client::Coordinate(3, 8)));
    nebula::client::Geography geogPoint2(
            nebula::client::Point(nebula::client::Coordinate(4, 6)));
    nebula::client::Geography geogLineString(
            nebula::client::LineString(std::vector<nebula::client::Coordinate>{
                    nebula::client::Coordinate(0, 1), nebula::client::Coordinate(2, 3)}));
    nebula::client::Geography geogPolygon(
            nebula::client::Polygon(std::vector<std::vector<nebula::client::Coordinate>>{
                    std::vector<nebula::client::Coordinate>{
                            nebula::client::Coordinate(0, 1),
                            nebula::client::Coordinate(1, 2),
                            nebula::client::Coordinate(2, 3),
                            nebula::client::Coordinate(0, 1)}}));
    expected.emplace_back(
            nebula::client::List({geogPoint1, geogPoint2, geogLineString, geogPolygon}));
...
TEST_F(SessionTest, Timeout) {
    nebula::client::ConnectionPool pool;
    nebula::client::Config c{10, 0, 100, 0, "", false};
    pool.init({kServerHost ":9669"}, c);
    auto session = pool.getSession("root", "nebula");
    ASSERT_TRUE(session.valid());
    ...
    ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::E_RPC_FAILURE) << *resp.errorMsg;
...
TEST_F(SessionTest, JsonResult) {
    nebula::client::ConnectionPool pool;
    nebula::client::Config c{10, 0, 10, 0, "", false};
    pool.init({kServerHost ":9669"}, c);
    auto session = pool.getSession("root", "nebula");
    ASSERT_TRUE(session.valid());
...
TEST_F(SessionTest, DurationResult) {
    ...
    nebula::client::DataSet expected({"d"});
    expected.emplace_back(
            nebula::client::Row({nebula::client::Duration().addYears(1).addSeconds(2)}));
    EXPECT_TRUE(verifyResultWithoutOrder(*resp.data, expected));
}
...
TEST_F(SessionTest, ExecuteParameter) {
    ...
    nebula::client::DataSet expected({"var"});
    expected.emplace_back(nebula::client::Row({1}));
    EXPECT_TRUE(verifyResultWithoutOrder(*resp.data, expected));
    ...
    session.asyncExecuteWithParameter(
            ...
            [&b, expected = std::move(expected)](auto&& aResp) {
                ASSERT_EQ(aResp.errorCode, nebula::client::ErrorCode::SUCCEEDED);
