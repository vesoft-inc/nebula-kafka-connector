/* Copyright (c) 2021 vesoft inc. All rights reserved.
 *
 * This source code is licensed under Apache 2.0 License.
 */

#include <common/Init.h>
#include <folly/synchronization/Baton.h>
#include <glog/logging.h>
#include <gtest/gtest.h>
#include <gtest/gtest_prod.h>
#include <nebula/client/ConnectionPool.h>
#include <nebula/client/Session.h>

#include <atomic>
#include <chrono>
#include <thread>

#include "./ClientTest.h"

// Require a nebula server could access

#define kServerHost "graphd"

class SessionTest : public ClientTest {};

TEST_F(SessionTest, Basic) {
    nebula::client::ConnectionPool pool;
    pool.init({kServerHost ":9669"}, nebula::client::Config{});
    auto session = pool.getSession("root", "nebula");
    ASSERT_TRUE(session.valid());
    {
        auto resp = session.execute(
                "YIELD datetime(\"2019-03-22T22:04:30\") as dt,"
                "date(\"2019-03-22\") as d, "
                "time(\"22:04:30\") as t");
        nebula::client::DataSet data({"dt", "d", "t"});
        data.emplace_back(
                nebula::client::Row({nebula::client::DateTime(2019, 3, 22, 22, 4, 30, 0),
                                     nebula::client::Date(2019, 3, 22),
                                     nebula::client::Time(22, 4, 30, 0)}));
        EXPECT_TRUE(verifyResultWithoutOrder(resp, data));

        // conversion
        session.toLocal(*resp.data);
        EXPECT_TRUE(verifyResultWithoutOrder(*resp.data, data));
    }
    {
        auto resp = session.execute(
                "YIELD datetime(\"2019-03-22T22:04:30\") as dt,"
                "date(\"2019-03-22\") as d, "
                "time(\"22:04:30\") as t");
        nebula::client::DataSet data({"dt", "d", "t"});
        data.emplace_back(
                nebula::client::Row({nebula::client::DateTime(2019, 3, 22, 22, 4, 30, 0),
                                     nebula::client::Date(2019, 3, 22),
                                     nebula::client::Time(22, 4, 30, 0)}));
        EXPECT_TRUE(verifyResultWithoutOrder(resp, data));

        // conversion
        session.toLocal(*resp.data, 30);
        nebula::client::DataSet expectLocal({"dt", "d", "t"});
        expectLocal.emplace_back(
                nebula::client::Row({nebula::client::DateTime(2019, 3, 22, 22, 5, 0, 0),
                                     nebula::client::Date(2019, 3, 22),
                                     nebula::client::Time(22, 5, 0, 0)}));
        EXPECT_TRUE(verifyResultWithoutOrder(*resp.data, expectLocal));
    }
}

int main(int argc, char** argv) {
    testing::InitGoogleTest(&argc, argv);
    nebula::client::init(&argc, &argv);
    google::SetStderrLogging(google::INFO);

    return RUN_ALL_TESTS();
}
