...
protected:
    static void runOnce(nebula::client::Connection &c) {
        // ping
        EXPECT_FALSE(c.ping());

        // execute
        auto resp = c.execute(-1, "YIELD 1");
        ASSERT_TRUE(resp.errorCode == nebula::client::ErrorCode::E_DISCONNECTED ||
                    resp.errorCode == nebula::client::ErrorCode::E_RPC_FAILURE);
        EXPECT_EQ(resp.data, nullptr);

        // async execute
        c.asyncExecute(-1, "YIELD 1", [](auto &&cbResp) {
            ASSERT_TRUE(cbResp.errorCode == nebula::client::ErrorCode::E_DISCONNECTED ||
                        cbResp.errorCode == nebula::client::ErrorCode::E_RPC_FAILURE);
        ...
        ASSERT_EQ(authResp.errorCode, nebula::client::ErrorCode::SUCCEEDED)
                << *authResp.errorMsg;
        ...
        c.asyncExecute(*authResp.sessionId, "YIELD 1", [&b](auto &&cbResp) {
            ASSERT_EQ(cbResp.errorCode, nebula::client::ErrorCode::SUCCEEDED)
        ...
        // check signout
        resp = c.execute(*authResp.sessionId, "YIELD 1");
        ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::E_SESSION_INVALID);

        // async execute
        folly::Baton<> b1;
        c.asyncExecute(*authResp.sessionId, "YIELD 1", [&b1](auto &&cbResp) {
            ASSERT_EQ(cbResp.errorCode, nebula::client::ErrorCode::E_SESSION_INVALID);
            b1.post();
        });
        b1.wait();
        ...
        // execute
        resp = c.execute(*authResp.sessionId, "YIELD 1");
        ASSERT_EQ(resp.errorCode, nebula::client::ErrorCode::E_DISCONNECTED);

        // async execute
        folly::Baton<> b2;
        c.asyncExecute(*authResp.sessionId, "YIELD 1", [&b2](auto &&cbResp) {
            ASSERT_EQ(cbResp.errorCode, nebula::client::ErrorCode::E_DISCONNECTED);
            b2.post();
        });
        b2.wait();
