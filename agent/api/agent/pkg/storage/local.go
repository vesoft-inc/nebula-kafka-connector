...
func (h *LocalConfig) DeepCopy() *LocalConfig {
	cp := &LocalConfig{
		Path: h.Path,
	}
	return cp
...
func NewLocal(b *Backend) (ExternalStorage, error) {
	if b.Type() != LocalType {
		return nil, fmt.Errorf("bad format local uri: %s", b.Uri())
	}
...
func (l *Local) Upload(ctx context.Context, externalUri, localPath string, recursively bool) error {
	// check external uri
	if ParseType(externalUri) != LocalType {
		return fmt.Errorf("invalid local uri: %s", externalUri)
	}
	dstPath := strings.TrimPrefix(externalUri, LocalPrefix)
...
func (l *Local) Download(ctx context.Context, localPath, externalUri string, recursively bool) error {
	...
	srcPath := strings.TrimPrefix(externalUri, LocalPrefix)
	srcInfo, err := os.Stat(srcPath)
	if os.IsNotExist(err) {
...
func (l *Local) ExistDir(ctx context.Context, uri string) bool {
	if ParseType(uri) != LocalType {
		log.Errorf("Invalid local uri type: %s.", uri)
		return false
	}

	p := strings.TrimPrefix(uri, LocalPrefix)
	exist, err := IsExist(p)
	if err != nil {
...
func (l *Local) GetDir(ctx context.Context, uri string) (*Backend, error) {
	if ParseType(uri) != LocalType {
