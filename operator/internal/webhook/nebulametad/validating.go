...
func (h *ValidatingAdmission) Handle(_ context.Context, req admission.Request) (resp admission.Response) {
	...
	if operation == admissionv1.Create {
		if allErrs := validateNebulaMetadCreate(obj); len(allErrs) > 0 {
		...
		if allErrs := validateNebulaMetadUpdate(obj, oldObj); len(allErrs) > 0 {
			return admission.Errored(http.StatusUnprocessableEntity, allErrs.ToAggregate())
		}
	} else if operation == admissionv1.Delete {
		if allErrs := validateNebulaMetadDelete(obj); len(allErrs) > 0 {
			return admission.Errored(http.StatusUnprocessableEntity, allErrs.ToAggregate())
		}
	}
